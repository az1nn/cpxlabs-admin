...
8. `passed` means only that every frozen validation command exited 0 and all V7 evidence invariants hold for the recorded worktree/run identity.
9. V7 binds each record to the worktree Git revision and deterministic pre/post SHA-256 fingerprints over the tracked diff and non-ignored untracked files; workspace drift during validation prevents `passed`.
10. Validation success does not mark a Spec Kit Task complete, approve review, release a lease, remove a worktree, alter Neo4j, or publish Git state.
11. Commit/push/PR/merge automation requires a separate future spec/ADR and must consume explicit fresh validation evidence rather than inheriting V7 authority implicitly.
...
- validation evidence is inspectable, workspace-bound and disposable;
- future publication automation gets a narrow evidence contract without being embedded in validation.

### Trade-offs

- a new validation is required after code/workspace changes that invalidate prior evidence;
- validations that intentionally rewrite publishable files fail the stable-workspace invariant and must be followed by a fresh validation after those changes are accepted;
...
### Ignore workspace drift during validation

Rejected. A future consumer could otherwise publish files different from those that actually passed validation.
